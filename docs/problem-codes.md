...
| `bad_redirect` | `BadRedirect` | a `$r->redirect()` route cannot answer: its status is not a redirect, or its target is unknown, does not answer GET, is itself a redirect, or has a param the redirect's path does not capture with the same type | name the target, the param and the placeholder to write |
