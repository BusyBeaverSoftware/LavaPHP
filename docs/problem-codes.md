...
| `bad_query` | `BadQuery` (db) | a query was built with something the builder cannot express — something other than a column name where one goes (an expression in `select()`, a condition, `orderBy()`, a join, or a write's keys), an alias that is not one name, two selected columns that would come back under one name, `count()` of a write, `IS NULL` as an equality, an array or object bound as a value, an empty `IN`, an empty where fragment, an empty `whereGroup` closure, a read verb in a write statement, a negative limit | name the column and the builder call to use instead |
