...
| `unexpected_failure` | `UnexpectedFailure` | a non-LavaProblem throwable escaped during boot (e.g. a PHP error in a user config file), while running a command, or while answering a request (a handler, a middleware, a subject resolver) | wrap the original class, message and location in the context, never the sentence or fix; a request answers 500, and in `prod` the response withholds the context and the app logs it (a boot failure has no logger: the skeleton's `public/index.php` writes it to the server's error log) |
| `bad_route_pattern` | `BadRoutePattern` | a route path is malformed (no leading `/`, untyped `{param}`, unknown/duplicate param type, unclosed placeholder), a custom type's fragment is not a valid regex, a route's types do not compile together, or URL generation passed a value the route would never match | quote the path and the exact syntax to write |
| `bad_request_path` | `BadRequestPath` | the request path, once decoded, holds a `.`/`..` segment or a control byte — shapes a conforming client cannot send literally, and which mean different things to this app and to a proxy in front of it | send the resolved path, with printable bytes |
| `bad_redirect` | `BadRedirect` | a `$r->redirect()` route cannot answer: its status is not a redirect, or its target is unknown, does not answer GET, is itself a redirect, has a param the redirect's path does not capture with the same type, or matches its target's own URLs; or, at request time, it leads to the address it was asked for | name the target, the param and the placeholder to write |
...
| `request_too_large` | `RequestTooLarge` | a form post larger than PHP's `post_max_size`, which PHP discarded before the app ran: a form content type, no fields, no files, and a `Content-Length` over the limit | send less, or raise `post_max_size` in the deployment's php.ini |
...
| `invalid_command_name` | `InvalidCommandName` | a command's name is not lowercase letters and digits joined by colons, so its `lava.<name>/N` envelope id would fail the envelope's own pattern — **severity warn** (the command still runs) | rename it; the fix names the nearest valid name no other command has, and none for a name outside ASCII; a command app/Commands.php added reads as `app` and points at its class |
...
| `bad_query` | `BadQuery` (db) | a query was built with something the builder cannot express — something other than a column name where one goes (an expression in `select()`, a condition, `orderBy()`, a join, or a write's keys), an alias that is not one name, two selected columns that would come back under one name (two column references alike apart from case included, since SQLite returns each under its table's declared name), `count()` of a write, `IS NULL` as an equality, an array or object bound as a value, an empty `IN`, an empty where fragment, an empty `whereGroup` closure, a read verb in a write statement, a negative limit | name the column and the builder call to use instead |
| `bad_schema` | `BadSchema` (db) | a table definition is invalid — empty or duplicate column, `autoIncrement` on a non-integer or on a composite key, an index on a column that isn't declared, a primary key in `table()`, `dropIndex()` naming an index the table does not have, a non-literal default, an inline FK on MySQL | name the column/index and the declaration to write |
...
| `invalid_listeners_file` | `InvalidListenersFile` (events) | `app/Listeners.php` does not parse, raises an `Error` while it is read, or does not return a map from event class names to a listener id or a non-empty list of ids | quote the file (at the error's line) and the shape to write |
| `bad_listener` | `BadListener` (events) | an `app/Listeners.php` entry names an event class or interface that does not exist, or a listener with no `__invoke()`, whose first parameter is not typed as the event (a parent, an interface it implements, or `object` also do), or that requires a second parameter | name the listener, the event and the signature to write |
| `factory_listener` | `FactoryListener` (events) | a listener is registered with `factory()`, and a listener is built once at boot, so every dispatch reaches one shared instance | register it with `singleton()` and keep per-dispatch state on the event |
| `listener_order_conflict` | `ListenerOrderConflict` (events) | one listener is given two phases in `app/Listeners.php` — by two entries, by one entry twice, or by a `Phase::first()`/`Phase::last()` beside a bare id, which is the default phase | name the listener, both phases and the entries that gave them |
...
| `autoescape_disabled` | `AutoescapeDisabled` (view) | something called `setDefaultStrategy()` on Twig's escaper, so the renderer would send unescaped values — the pack promises escaping is on and not configurable | name the template, and the per-value and per-template opt-outs that are the supported ones |
...
| `response_too_large` | `ResponseTooLarge` (http-client) | the response body passed `max_response_bytes` and the transfer was abandoned — the ceiling exists because a body is buffered in memory, and without one an upstream can end the request in a fatal error no problem can catch. **Not retried** | raise the ceiling if a body that size is expected; if it is not, the URL is answering with something other than the API resource |
| `unsendable_request` | `UnsendableRequest` (http-client) | the method or a header carries a line break or a null byte, which would end the request line and put a second request of someone else's choosing on the connection. **Not retried** | strip CR, LF and NUL — a method or header forwarded from an inbound request is the usual source, and must be checked against a list the app chooses |
