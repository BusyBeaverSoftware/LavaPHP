...
| `bad_schema` | `BadSchema` (db) | a table definition is invalid — empty or duplicate column, `autoIncrement` on a non-integer or on a composite key, an index on a column that isn't declared, a primary key in `table()`, a non-literal default, an inline FK on MySQL | name the column/index and the declaration to write |
