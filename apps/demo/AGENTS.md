<!-- lava:map hash=1dbe94356508bb22 -->
# AGENTS.md
...
| package | feature | module | declared at |
|-------|-------|------|-----------|
| lavaphp/db | db | DbModule | app/Modules.php:27 |
| lavaphp/validate | validate | ValidateModule | app/Modules.php:28 |
| lavaphp/view | views | ViewModule | app/Modules.php:29 |
| lavaphp/http-client | http_client | HttpClientModule | app/Modules.php:30 |
| lavaphp/events | events | EventsModule | app/Modules.php:31 |
...
| GET | /tasks/export | tasks.export | App\Http\TasksController::export | tasks_csv_export | - |
| GET | /tasks | tasks.index | App\Http\TasksController::index | - | - |
| POST | /tasks | tasks.store | App\Http\TasksController::store | - | - |
| GET | /tasks/{id:int} | tasks.show | App\Http\TasksController::show | - | - |
| POST | /tasks/{id:int}/complete | tasks.complete | App\Http\TasksController::complete | - | - |
| DELETE | /tasks/{id:int} | tasks.destroy | App\Http\TasksController::destroy | - | - |
| GET | / | home | App\Http\TasksPageController::index | - | - |
| GET | /tasks/{id:int}/view | tasks.page | App\Http\TasksPageController::show | - | - |
| GET | /upstream/health | upstream.health | App\Http\UpstreamController::health | - | - |
...
| Lava\Db\Connection | singleton | Lava\Db\Connection | db:src/DbModule.php:52 |
| Lava\View\ViewRenderer | singleton | Lava\View\ViewRenderer | view:src/ViewModule.php:87 |
| Lava\HttpClient\ClientOptions | singleton | Lava\HttpClient\ClientOptions | http-client:src/HttpClientModule.php:67 |
| Lava\HttpClient\CurlTransport | singleton | Lava\HttpClient\CurlTransport | http-client:src/HttpClientModule.php:71 |
| Lava\HttpClient\HttpClient | singleton | Lava\HttpClient\HttpClient | http-client:src/HttpClientModule.php:76 |
| Lava\Events\ListenerMap | singleton | Lava\Events\ListenerMap | events:src/EventsModule.php:74 |
| Lava\Events\ListenerProvider | singleton | Lava\Events\ListenerProvider | events:src/EventsModule.php:76 |
| Lava\Events\EventDispatcher | singleton | Lava\Events\EventDispatcher | events:src/EventsModule.php:106 |
| App\Tasks\TaskRepository | singleton | App\Tasks\TaskRepository | app/Services.php:40 |
| App\Upstream\Upstream | singleton | App\Upstream\Upstream | app/Services.php:53 |
| App\Tasks\LogCompletion | singleton | App\Tasks\LogCompletion | app/Services.php:63 |
| App\Http\RequestIdMiddleware | singleton | App\Http\RequestIdMiddleware | app/Services.php:74 |
| app.env_vars | value | - | app/Services.php:82 |
...
|----|-------|----|-----------|
| db | on | lavaphp/db | gate for pack lavaphp/db |
| validate | on | lavaphp/validate | gate for pack lavaphp/validate |
| views | on | lavaphp/view | gate for pack lavaphp/view |
| http_client | on | lavaphp/http-client | gate for pack lavaphp/http-client |
| events | on | lavaphp/events | gate for pack lavaphp/events |
| tasks_csv_export | off | - | The GET /tasks/export route |
...
| db:status | db | Show applied and pending migrations, and any the repository remembers without a file. |
| db:migrate | db | Apply every pending migration as one batch. |
| db:rollback | db | Undo the last batch of migrations (or the last N with --batches). |
| db:new | db | Write a new migration file with the next timestamp. |
| events | events | List each event in app/Listeners.php and the listeners it reaches, in order. |
| app:seed | app | Insert a few sample tasks. |
| app:stats | app | Count the tasks, and how many are open, done or overdue. |

## Events (1)

Each event class or interface in app/Listeners.php and the listeners it reaches, in the order they run. An event reaches the listeners of every entry it is an instance of; dispatch with Lava\Events\EventDispatcher. A listener marked (first) or (last) runs before or after the unmarked ones, whichever entry names it. Listed by lavaphp/events.

| event | listeners |
|-----|---------|
| App\Tasks\TaskCompleted | App\Tasks\LogCompletion |
...
| name | requirement | secret | declared by | purpose |
|----|-----------|------|-----------|-------|
| DATABASE_DSN | optional | - | lavaphp/db | Read by lavaphp/db. |
| DATABASE_USER | optional | - | lavaphp/db | Read by lavaphp/db. |
| DATABASE_PASSWORD | optional | - | lavaphp/db | Read by lavaphp/db. |
| UPSTREAM_URL | optional | - | app/Services.php | What GET /upstream/health fetches. Defaults to this app's own /health. |
...
| # | middleware |
|---|----------|
| 1 | App\Http\RequestIdMiddleware |
